...
function serveSiteCards(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      // Comodità: la base dev è /simulator/, quindi / e /simulator da soli
      // non esistono — rimanda chi ci arriva all'indirizzo giusto.
      server.middlewares.use((request, response, next) => {
        const path = (request.url ?? "").split("?")[0];
        if (path === "/" || path === "/simulator") {
          response.statusCode = 302;
          response.setHeader("Location", "/simulator/");
          return response.end();
        }
        next();
      });
...
export default defineConfig(({ command }) => ({
  // In sviluppo la pagina sta a /simulator/ e le carte a /cards (il
