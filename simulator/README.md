...
```bash
npm install          # solo la prima volta, dalla radice (workspace)
cd simulator
npm run dev          # la pagina: http://localhost:5199/simulator/
```
...
di produzione in `core/src/engine.ts` (`PRODUCTION_ENGINE`, usato da
`defaultEngineUrl`): se Render assegna un nome diverso, va aggiornato lì. Nota del piano free: il servizio dormirebbe dopo un quarto
...
La vista **Mazzi** (`src/decks.ts`, sezione `#decks`) è per chi gioca, non è
...
`core/src/bot.ts`, senza DOM (provabili con uno stato finto, `core/test/bot.test.ts`):
...
La guida sta nella sessione, `core/src/session.ts` (`botTick`), comune ai
due client: a ogni ridisegno, se è il suo
...
la sua copia della partita e applica le stesse mutazioni (`core/src/state.ts`),
...
ricostruisce la lavagna da lì (`replay` in `core/src/state.ts`): lo stato è una
...
(`core/src/engine.ts`, guidato da `core/src/session.ts`). Ogni azione
aspetta il verdetto prima di applicarsi, e
...
mostra lo stato). L'indirizzo è quello di `defaultEngineUrl` in
`core/src/engine.ts` (calcolato in `src/main.ts`), o
