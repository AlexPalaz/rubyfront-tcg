...
carte, il sito che le mostra, il gioco per giocarle online e l'engine
...
flowchart LR
  ...
  subgraph client["core/ + game/ (TypeScript, nel browser)"]
    ST["core: state.ts<br/>il riduttore"]
    TB["game: table.ts · scene.ts<br/>il tavolo (PixiJS)"]
  ...
  PEER["il gioco<br/>dell'avversario"]
...
| Pezzo | Legge | Quando |
|---|---|---|
| Sito e gioco | `docs/cards/catalog.json` | a ogni apertura della pagina |
...
Il gioco è il poliziotto, l'engine dà le regole. Ogni gesto locale si
...
|---|---|---|
| Gioco e catalogo | `npm run all` → il gioco PixiJS su `:5200` (carte su `/cards`) | Vercel, build a ogni push (`vercel.json` → `scripts/build-site.mjs`): il **gioco** alla radice `/` (`/next`, dove stava prima, rimanda lì), il **catalogo** sotto `/catalog`; esce `dist/`, non si committa |
...
```sh
npm run all                          # gioco + tavolo (engine), un Ctrl+C spegne tutto
...
cd game && npx vitest run            # il gioco: la geometria del tavolo, il modello delle carte
node scripts/build-site.mjs          # dist/: il gioco e il catalogo
```
