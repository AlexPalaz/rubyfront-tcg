// La geometria CANONICA della lavagna: le misure in cui le posizioni delle
// carte stanno nello stato e viaggiano in rete. Sono le stesse sui due
...
export const SWAP_Y = HALF_H + HALF_GAP;

/** Ordinata canonica di ciascuna fascia. */
export const BAND_TOP: Record<Seat, number> = { b: TOP_PAD, a: TOP_PAD + SWAP_Y };
 ...
 */
export const HALF_TILE = TILE_H / 2;

/** In quale fascia cade un centro (null: fuori da entrambe). */
export function bandOfCenter(center: number): Seat | null {
 ...
 */
export function canonBandTop(seat: Seat, viewer: Seat): number {
 ...
 */
export function flipInBand(localCenter: number): number {
  return HALF_H - localCenter;
}

/** In quale fila della fascia cade una carta: Fronte o fila di servizio. */
export function rowOfLocal(local: number): "front" | "back" {
