...
const CARDS: Record<string, Partial<CardFacts>> = {
  BIG: { kind: "entity", race: "human", power: 4, fluxCost: 4 },
  MEDIUM: { kind: "entity", race: "human", power: 2, fluxCost: 2 },
  SMALL: { kind: "entity", race: "human", power: 1, fluxCost: 1 },
  ...
  CROWN: {
    ...
    furyAt: { 0: 13 },
    abilities: [
      { id: "glance", displayKey: "glance", face: 0, timing: ["preparazione", "fronte"], cost: null, gain: 3, fury: true, form: { kind: "look", count: 3, reveal: { kind: "entity", race: "human" } } },
      ...
      { id: "manual", displayKey: "manual", face: 0, timing: ["preparazione"], cost: 7, gain: null, fury: false, form: null },
    ],
...
describe("attacchi", () => {
  it("attacca con chi non muore, tiene a casa chi verrebbe ucciso", () => {
    ...
    const big = put(state, "BIG", "b", "field");
    const small = put(state, "SMALL", "b", "field");
  ...
  it("va all'assalto quando il colpo che passa è letale", () => {
    ...
    put(state, "MEDIUM", "b", "field");
    put(state, "MEDIUM", "b", "field");
    put(state, "BIG", "a", "field");
    expect(chooseAttackers(state, "b", facts, freshMemory(1))).toHaveLength(2);
  });

  it("rispetta l'attesa di evocazione e salta le tappate", () => {
    const state = newGame("b");
    const fresh = put(state, "BIG", "b", "field");
    const tapped = put(state, "BIG", "b", "field", { tapped: true });
...
describe("blocchi", () => {
  it("blocca con un muro, contrattacca per uccidere, sacrifica solo in pericolo", () => {
    ...
    const ruby = put(state, "RUBY", "b", "field");
    const a1 = put(state, "BIG", "a", "field");
    const a2 = put(state, "MEDIUM", "a", "field");
    attack(state, a1, ruby, 1);
    attack(state, a2, ruby, 2);
    const lance = put(state, "SPEAR", "b", "field");
    const small = put(state, "SMALL", "b", "field");
    const blocks = chooseBlocks(state, "b", facts);
    // La lancia (1+3=4) pareggia BIG: non lo supera, quindi non è un
    ...
    // GRANDE (4) contro SMALL: nessun muro, i PV sono 20, nessun sacrificio.
...
describe("cosa giocare", () => {
  it("gioca l'Entità più forte che il Flusso copre, col Gettone se serve un Flusso in più", () => {
    ...
    put(state, "BIG", "b", "hand");
    const media = put(state, "MEDIUM", "b", "hand");
    const play = choosePlay(state, "b", facts, freshMemory(1));
    expect(play?.kind).toBe("entity");
    expect(play?.card.cardId).toBe("BIG");
  ...
  it("non insiste su una carta fermata", () => {
    ...
    const big = put(state, "BIG", "b", "hand");
    const memory = freshMemory(1);
...
describe("scarti e scelte", () => {
  it("scarta fino a 7, le carte che valgono meno", () => {
    const state = newGame("b");
    for (let i = 0; i < 8; i += 1) put(state, "BIG", "b", "hand");
    put(state, "SMALL", "b", "hand");
    const discards = chooseDiscards(state, "b", facts);
    expect(discards).toHaveLength(2);
    expect(discards[0].cardId).toBe("SMALL");
  });

  it("pickBest colpisce l'avversario più forte, potenzia il proprio più forte, scarta il più debole", () => {
    const state = newGame("b");
    const big = put(state, "BIG", "a", "field");
    const small = put(state, "SMALL", "a", "field");
    const mine = put(state, "MEDIUM", "b", "field");
...
describe("chooseResponse (§7.2, la catena)", () => {
  it("risponde con la Reattiva che agisce e che paga, mai con la bloccante; senza, accetta", () => {
    ...
    put(state, "SMALL", "b", "field");
    const burst = put(state, "BURST", "b", "hand");
    put(state, "SHELTER", "b", "hand");
    expect(chooseResponse(state, "b", facts)?.card.uid).toBe(burst.uid);
...
describe("chooseAbility e chooseFlip (§3.1)", () => {
  ...
  it("paga il potenziamento a ondata dichiarata quando i punti comprati valgono il prezzo", () => {
    ...
    state.phase = "fronte";
    const memory = freshMemory(4);
    const foe = put(state, "RUBY", "a", "field");
    const attackers = [put(state, "MEDIUM", "b", "field"), put(state, "MEDIUM", "b", "field"), put(state, "MEDIUM", "b", "field")];
  ...
  it("paga sempre il colpo letale", () => {
    ...
    const foe = put(state, "RUBY", "a", "field");
    const one = put(state, "MEDIUM", "b", "field");
