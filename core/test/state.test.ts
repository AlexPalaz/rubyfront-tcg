...
describe("apply turn", () => {
  ...
  it("il ritorno vincolato (§8.2) rimette l'Entità sullo slot e le mette addosso l'Oggetto dal Ritiro — gemello: table_test.rb", () => {
    ...
    const same = apply(state, { t: "revive", uid: entity, x: 442, y: frontRowY("a"), z: 9, object: "missing", effect: { source: entity, event: "on_leave_field", entering: entity } });
...
describe("attrezzi degli effetti d'attacco", () => {
  ...
  it("refresh stappa chi comanda il posto solo col tiro giusto (RBF-011)", () => {
    ...
    const entering = { source: "a1", event: "on_enter_field" as const, entering: "a1" };
    const next = apply(state, { t: "refresh", seat: "a", roll: 17, untap: true, effect: entering });
...
describe("apply ability / sconti", () => {
  it("paga i PV, potenzia i bersagli e tiene lo sconto finché non si gioca", () => {
    ...
    state = apply(state, { t: "ability", uid: "a-2", ability: "glance", gain: 3 });
    expect(state.players.a.hp).toBe(17);
    state = apply(state, { t: "ability", uid: "a-2", ability: "discount", cost: 3, discount: { amount: 1, type: "object", race: null } });
  ...
  it("la chiamata sul Fronte: promessa, discesa con Slancio, bonus all'attacco", () => {
    ...
    state = apply(state, { t: "toZone", uid: "a-2", zone: "hand" });
    state = apply(state, { t: "toZone", uid: "a-3", zone: "hand" });
    state = apply(state, { t: "ability", uid: "a-1", ability: "call", cost: 7, bonus: { amount: 1, race: "human" } });
    expect(state.players.a.hp).toBe(14);
    ...
    state = apply(state, { t: "toZone", uid: "a-2", zone: "field", x: 442, y: 1260, z: 1, grants: ["surge"], effect: { source: "a-1", event: "on_ability", entering: "a-1", ability: "call" } });
    expect(state.cards["a-2"].zone).toBe("field");
