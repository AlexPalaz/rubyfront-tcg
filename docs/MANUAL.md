...
**Only a card can send it back to the Recall Zone** (golden rule, §1.1). No card does so today; if one does, from the moment of the return the Rubyfront stops enabling its Matters (the static cards of the types that only it enabled go to the Abyss, §7.2) and the redeployment pays the full cost again — a new roll, if the cost is a die.
...
- use its **special abilities** (paying their HP cost) — activated **freely, only on your own turn**, in the Preparation Phase as well as the Front Phase. You may use **only one special ability per turn**, whichever it is; flipping to the Nexus **reopens the window**: on the turn it flips, the player may also use a Nexus ability, after the one already used on the Rubyfront. They resolve **with no chance of response** (the chain applies only to Reactives). An ability can be activated **only if the HP cover the whole cost** (HP ≥ cost): HP never go below 0. **Beware:** paying down to exactly 0 is legal, but at 0 HP you lose the game **immediately** — the ability's effect **does not resolve**;
...
- **HP recovered:** the Nexus face shows an **HP recovery** (e.g. `+3`): the Nexus starts with the **HP the Rubyfront had left plus the recovery shown**.
...
- **Front** — the battlefield: **5 slots** where Entities stand. The Rubyfront, when deployed, is placed **in front of** the 5 slots (it doesn't take up a slot); Matters in play (Static or resolving) stand **behind** the 5 slots. Statics are lined up **one behind the other (or one below the other), in the order in which they came onto the field**: the row keeps track of each one's age, which is needed for the resolution order of effects (§8.2). Matters **are not played on the Front slots**: wherever possible, they are laid **in the Matters space**, behind the slots — the slots belong to Entities, and the row of Matters must stay legible because it's what tells each one's age. An Entity **occupies the slot it came down on**: it doesn't move from one slot to another, unless a card says so (golden rule, §1.1).
- **Deck** — the cards to draw.
- **Abyss** — the zone of **dead or spent** cards: Entities that died or were destroyed, Matters that resolved, decayed or vanished — not the Objects of a dead Entity, which go to the Retire Zone (§3.1). It is **public**: either player may look through it at any time.
- **Retire Zone** — the zone of cards **still "alive"** that have left the game: Entities that were **retired** (§6.2) or **sent there by card effects**, **cards discarded from hand** (for excess, §6.5, or through an effect), and the Objects that follow the Entities off the field — dead ones included (§3.1) —, Objects that **stay** there even when the Entity comes back onto the field. It works **exactly like the Abyss** (public, viewable at any time), but keeps apart what never died from what did: effects that name the Abyss **do not** touch the Retire Zone, and vice versa. A card sent to the Retire Zone by an effect **does not die** and, unless the effect says otherwise, **does not count as a Retire**: it only counts as **leaving the field**. **There is no moving between the two zones by hand:** a card goes from the Retire Zone to the Abyss only if an effect sends it there. The Retire Zone is reached **from the Front** (by retiring, §6.2, or through an effect) and **from the hand** (by discarding, §6.5); the Abyss is reached **by dying** (or by being spent, for resolved Matters).
...
- **play** normal and Static **Matters** (Reactives are played only in the Front Phase, §7.2), laying them **in the Matters space**, never on the Front slots (§5);
...
On the Front you may have **at most 5 Entities at the same time** — the **5 slots** of the Front (§5). **Only Entities** count towards the limit: the **Rubyfront** takes no slot (deployed, it stands in front of the slots), **Static Matters** stand behind the slots and **have no limit in number** (the only constraint is enabling, §7), **Objects** are assigned to Entities and take no slot.
...
- **Assigned Object:** it follows its Entity (§3.1) — it too goes to the **Retire Zone**. An Object **doesn't retire on its own**: it reaches the Retire Zone **only by following the Entity** it is assigned to (or through the effect of a card: a disarm, or the Object itself saying it may be put into the Retire Zone by paying its cost). If the Entity **comes back onto the field**, though, it comes back **without Objects**: those stay in the Retire Zone until a new assignment — from the hand, paying, or through the effect of a card that brings them back onto the field from the zone — puts them back into play (§3.1).
- **Enabling Matters:** the retired Entity stops enabling its Matter types. If it was the last one enabling a type, access is lost and the Static Matters of that type go to the Abyss (§7.2). Retiring without checking what you are enabling is a costly mistake.
- **A tapped or covered Entity cannot be retired.** Retiring requires an **untapped** Entity: a covered one is untouchable even for its owner (§6.3), a tapped one is busy and doesn't retire until it untaps. This also applies to **stasis** (§8.1), which is a static card tap: an Entity in stasis **cannot be retired** and keeps taking up its slot until it is untapped by an effect or leaves the field some other way. In practice, on your own turn your Entities untap at the start of the turn, so the constraint weighs on Entities in stasis and on those tapped by effects.
...
You may not have **more than 7 cards in hand**: at the end of your own turn, the excess cards are **discarded** (to the **Retire Zone**, §5: a discard is not a death).
...
- The card is **Static**: it **stays in play** and doesn't go to the Abyss.
- **It doesn't take up a Front slot:** it stands **behind** the 5 Entity slots (§5) and doesn't count towards the limit; there is no limit to the number of static cards in play.
- **It decays together with its enabling:** if the player loses access to the Matter type **at the static card's grade** (the last card enabling that type at the required grade leaves the field), the static card goes to the **Abyss**. A lower-grade enabler **is not enough**: a second-grade Static decays if only a first-grade enabler is left on the field — consistent with the response chain, where enabling is always rechecked "at the required grade". **Exception — cover:** if access is lost only because the last enabling card has been **covered** (§6.3), the static card doesn't go to the Abyss: **it is covered too** — it stays on the field with its effect suspended, as if it weren't on the field — and **reactivates** when the enabler is uncovered. If the enabler leaves the field while covered, the covered Static goes to the Abyss. **This applies to the Rubyfront too** — which, however, never leaves the field by itself (§3.1): only if **a card** sends it back to the Recall Zone does it stop enabling its Matters, and the static cards of the types that **only it** enabled (typically Destructive) go to the Abyss; with the **flip to the Nexus**, the static cards of the types the Nexus face no longer enables decay (§3.1, "The Nexus").
...
*Example: I play an Entity with the entry effect "discard a card" while I have two static cards on the field — the older one says "when you play an Entity, draw a card", the more recent one "when you play an Entity, deal 1 damage to the opposing Rubyfront". Order: first the Entity that entered (discard), then the younger static card (damage), finally the older one (draw).*
...
- At most **5 Entities on the Front** (the 5 slots; the Rubyfront and Static Matters don't count, §6.2).
...
- **Abyss** — the zone of dead or spent cards: Entities that died or were destroyed, Matters that resolved, decayed or vanished (not the Objects of a dead Entity: they go to the Retire Zone). It is public.
- **Retire Zone** — the zone of cards still "alive" that have left the game: retired Entities (§6.2) or ones sent there by effects, cards discarded from hand (§6.5), and the Objects of Entities that left the field, dead ones included. It works like the Abyss, but sets apart what never died; effects that name one of the two zones don't touch the other.
...
- **Matter** — a spell/event card, playable only if a matching type is enabled by a card on the field. Three behaviours: normal, Static, Reactive.
