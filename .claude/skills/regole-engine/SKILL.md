...
| Trasporto | `engine/bin/server`, `engine/lib/rubyfront/websocket.rb` | Un thread per client, una stanza per partita (`?room=&seat=`). L'**utenza** (`Account` nel server, dal 2026-09-20) è della connessione: `login`/`logout`/`save`/`load`, prima della stanza. |
| Memoria | `engine/lib/rubyfront/store.rb` | Postgres su Neon per HTTPS (`/sql`, nessuna gemma): `players` e `player_data` (una riga per chiave, JSON). `DATABASE_URL` e `RUBYFRONT_DEV_TOKEN` dall'ambiente (`.env.local` in locale, ignorato dal repo). Il giudizio non la conosce. Test con trasporto finto (`store_test.rb`). |
